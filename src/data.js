...
// Local fallback for window.claude.complete so the AI chat works outside
// claude.ai/design. If a real implementation is injected by the host page
// it wins (we only define the shim when nothing is there).
if (!window.claude || typeof window.claude.complete !== 'function') {
  ...
  window.claude.complete = async function (prompt) {
    ...
    const q = (prompt.match(/ผู้ใช้ถาม:\s*"([^"]+)"/) || [, ''])[1].toLowerCase();

    const typeKeywords = {
      'EIA':       /eia|สิ่งแวดล้อม/i,
      'แบบ':       /แบบ|drawing|blueprint|สถาปัตย์|โครงสร้าง/i,
      'สัญญา':     /สัญญา|contract/i,
      'แรงงาน':    /แรงงาน|labor|safety|ปลอดภัย/i,
      'มาตรฐาน':   /มยผ|มาตรฐาน|standard|spec/i,
      'MOM':       /mom|ประชุม|minutes/i,
      'ราคา':      /ราคา|boq|ประมาณการ|budget/i,
    };
    let wantedType = null;
    for (const [t, re] of Object.entries(typeKeywords)) {
      if (re.test(q)) { wantedType = t; break; }
    }

    const proj = data.projects.find(p => q.includes(p.code.toLowerCase()))
              || data.projects.find(p => p.name.toLowerCase().split(/\s+/).some(w => w.length > 3 && q.includes(w.toLowerCase())))
              || null;

    const wantsZip = /zip|รวม|แพ็?ค|ดาวน์โหลดทั้งโครงการ|ทั้งหมด/.test(q);
    const wantsCount = /กี่|มีกี่|จำนวน|how many|count/.test(q);
    const wantsLatest = /ล่าสุด|latest|ปัจจุบัน/.test(q);
    ...
    if (wantsZip && proj) {
      return fmt({
        intent: 'download_zip',
        reply: `รวมไฟล์ทั้งโครงการ ${proj.code} (${proj.name}) เป็น Zip ให้ครับ — มีทั้งหมด ${data.files.filter(f => f.projectId === proj.id).length} ไฟล์`,
        fileName: null,
        projectCode: proj.code,
      });
    }

    if (wantsCount && wantedType) {
      const scope = proj ? data.files.filter(f => f.projectId === proj.id) : data.files;
      const n = scope.filter(f => f.type === wantedType).length;
      return fmt({
        intent: 'answer',
        reply: proj
          ? `${proj.code} มีเอกสารประเภท ${wantedType} ทั้งหมด ${n} ฉบับครับ`
          : `ระบบมีเอกสารประเภท ${wantedType} ทั้งหมด ${n} ฉบับครับ`,
        fileName: null, projectCode: null,
      });
    }

    if (wantedType || proj) {
      let candidates = data.files.slice();
      if (wantedType) candidates = candidates.filter(f => f.type === wantedType);
      if (proj) candidates = candidates.filter(f => f.projectId === proj.id);
      if (wantsLatest || candidates.length) {
        candidates = candidates.filter(f => f.isLatest);
      }
      ...
      if (hit) {
        return fmt({
          ...
          reply: `พบเอกสาร${wantedType ? ' ' + wantedType : ''}${proj ? ' ของ ' + proj.code : ''} Version ล่าสุดแล้วครับ — ${hit.name}`,
          fileName: hit.name,
          projectCode: null,
        });
      }
    ...
    return fmt({
      intent: 'answer',
      reply: `ระบบมีโครงการทั้งหมด ${totalProjects} โครงการ และไฟล์ ${totalFiles} ฉบับครับ ลองระบุชื่อโครงการ (เช่น MRT-PP) หรือประเภทเอกสาร (EIA / แบบ / BOQ / MOM) ดูครับ`,
      fileName: null, projectCode: null,
    });
  };
}
